...
- The final login page polish replaces CGI with CMIPS branding, removes developer-focused copy, strengthens demo buttons, modernizes the login card/inputs, and makes Continue a full-width primary gradient action consistent with the workspace.
- Matching Procedures copy now reflects the actual number of available matches: one match uses singular wording and two or more matches use plural wording, while zero matches continue to hide the panel and use the existing no-match experience.
- Keyword retrieval now normalizes searchable text by collapsing whitespace before indexing and phrase scoring, preserving original display text while allowing PDF-wrapped phrases such as `Direct\nDeposit` to match broad user queries like `Direct deposit`.
- Matching Procedures now displays every script candidate that passes the existing hybrid score threshold instead of applying a hardcoded top-5 cap. Large matching lists become scrollable in the UI while preserving score sort order, confidence threshold, and direct-answer behavior.
- Broad Matching Procedures searches now have UI guidance only: the panel shows the exact returned match count, displays a concise broad-search hint, derives refinement suggestions from returned procedure titles, and provides an instant client-side filter for already-returned procedures when more than 10 matches are present.
- Matching Procedures broad-result presentation now reduces overload without changing retrieval: it shows the total count, displays the first six ranked procedures as `Best Matches`, and keeps the remaining ranked procedures behind a collapsed `Show all` control with a scrollable expanded list.
- Matching Procedures final UX simplifies the broad-result assistant experience: only the top five ranked procedures are shown, search-within-results and show-all controls are removed, and suggested search chips remain as one-click refinements.
...
- On September 21, 2026, final login page polish was applied without changing authentication behavior. `npm run typecheck` passed, source scan confirmed the old developer copy was removed, and browser validation confirmed the page renders CMIPS branding, business-oriented product copy, demo actions, and the primary Continue action.
- On September 21, 2026, the Matching Procedures UX copy was corrected without retrieval or backend changes. `npm run typecheck` passed after replacing the hardcoded plural heading with count-aware singular/plural helper text.
- On September 21, 2026, keyword searchable-text normalization was added without changing hybrid weights or top-N limits. `npm run typecheck` and `npm run build:api` passed. The `Direct deposit` probe now gives `Where do I send my Direct Deposit form?` keyword score `89`, keyword rank `4`, hybrid rank `3`, and includes it in the chat `scenarioMatches`.
- On September 21, 2026, the Broad Query Guidance UI was added without retrieval, scoring, threshold, backend, or API changes. `npm run typecheck` passed. Live validation through the API and browser confirmed `Direct deposit` returns and renders 82 matching procedures, shows broad-search guidance, displays derived refinement chips, keeps all 82 returned procedures in the list, and filters the already-returned list client-side. `I forgot my ESP password` still returns a direct answer with zero `scenarioMatches`; the current live API returns 32 matches for `Paid sick leave`, so it correctly uses the broad matching-procedures experience rather than a direct answer.
- On September 21, 2026, the Matching Procedures top-5 cap was removed without lowering thresholds or changing hybrid scoring. `npm run typecheck` and `npm run build:api` passed. The `Direct deposit` probe now returns 82 qualifying `scenarioMatches`; all 45 indexed scenarios containing `Direct Deposit` are present, including `Where do I send my Direct Deposit form?` at rank 6.
- On September 21, 2026, Matching Procedures broad-result presentation was refined without retrieval, ranking, threshold, backend, or API changes. `npm run typecheck` passed. Browser validation for `Direct deposit` confirmed the collapsed panel shows `82 procedures found`, `Best Matches`, six visible top-ranked procedure cards, suggested searches, and search-within-results; expanding `Show all 82 matching procedures` reveals all 82 procedures with the remaining list starting at the seventh-ranked result.
- On September 21, 2026, Matching Procedures final UX was simplified without retrieval, ranking, threshold, backend, or API changes. `npm run typecheck` passed. Browser validation confirmed `Direct deposit` shows only five best matches, guidance, suggested search chips, no search-within-results, no show-all control, and no count heading. `Password` also shows five top password procedures with guidance. API validation showed `Forgot my ESP password` currently returns 36 scenario matches while `I forgot my ESP password` returns the direct-answer flow; this was left unchanged because retrieval/scoring/thresholds were frozen for this UX-only sprint.
